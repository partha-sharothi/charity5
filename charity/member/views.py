...
def user_activitys(self):
    ...
    if len(downline_number)>=2 and len(direct_users)>=1 and obj==0:
        ...
        # obj.end_date = obj.start_date + timedelta(days=7) 
        obj.end_date = obj.start_date + timedelta(minutes=45)
        obj.save()
...
#########------------>>>>> reward <<<<---------- end----#########
    ...
    return render(request,'index_dash.html',{'reward':reward, 'user_activity':user_activity,'referral_url':referral_url,'single_leg':single_leg,'points':points,'income':income,'total_income':total_income}) 
